...
class Fotografias(models.Model):
    ...
    foto = models.ImageField(
        upload_to='fotos/%Y/%m/%d/', blank=False
    )   # Imagem da fotografia, upload_to é o diretório onde a imagem será salva, no caso, na pasta galeria
    publicada = models.BooleanField(
        default=False
    )   # Se a fotografia está publicada ou não
    data_fotografia = models.DateTimeField(
        default=datetime.now, blank=False
    )   # Data da fotografia, auto_now_add adiciona a data atual quando o objeto é criado
